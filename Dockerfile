...
RUN \
 ...
 sed -i "s|Unsupported Third Party Container|linuxserver.io image version ${VERSION} - get support at https://linuxserver.io/support|" /usr/lib/python3.9/site-packages/homeassistant/helpers/system_info.py && \
 echo "**** cleanup ****" && \
 apk del --purge \
  build-dependencies && \
 rm -rf \
